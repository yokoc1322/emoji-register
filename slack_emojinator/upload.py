...
def _argparse(emoji_file):
    ...
    if not args['cookie']:
        args['cookie'] = raw_input(
            'Please enter the "emoji" cookie: ').strip()
    return args
...
def _fetch_api_token(session):
    ...
    raise Exception(
        'api_token not found. response status={}'.format(r.status_code))
...
def upload_emoji(session, emoji_name, filename):
    ...
    r = session.post(session.url_add, data=data,
